...
type TimeSeriesPredictionSpec struct {
	...
	// PredictionCycle is the prediction time series length, typically it is an cycle of your time series. which is at least 1d now.
	...
	PredictionCycle metav1.Duration `json:"predictionWindow,omitempty"`
	// RefreshFrequency is the update frequency of the TimeSeriesPrediction status
	...
	// Then next RefreshFrequency it will do prediction again and update the status of the crd.
	// RefreshFrequency must less than PredictionCycle. typically it is 1h, 30m, half of your PredictionCycle.
	// Do not set it too small, such as 1s, 1min, 5min, because the predicted data may has no changes in short periods.
	RefreshFrequency metav1.Duration	`json:"refreshFrequency,omitempty"`
}
...
type PredictionMetric struct {
	...
	Algorithm Algorithm `json:"algorithmType,omitempty"`
}
...
// String return sample string
func (s *Sample) String() string {
	return fmt.Sprintf("%v %v", s.Timestamp, s.Value)
}
...
// NewMetricTimeSeries return a MetricTimeSeries
func NewMetricTimeSeries() *MetricTimeSeries {
...
// SetLabels set the MetricTimeSeries labels
func (ts *MetricTimeSeries) SetLabels(labels []Label) {
	ts.Labels = labels
}

// SetSamples set the MetricTimeSeries samples
func (ts *MetricTimeSeries) SetSamples(samples []Sample) {
	ts.Samples = samples
}

// AppendLabel append a label to the MetricTimeSeries
func (ts *MetricTimeSeries) AppendLabel(key, val string) {
	ts.Labels = append(ts.Labels, Label{key, val})
}

// AppendSample append a sample to the MetricTimeSeries
func (ts *MetricTimeSeries) AppendSample(timestamp int64, val float64) {
	ts.Samples = append(ts.Samples, Sample{Timestamp: timestamp, Value: fmt.Sprintf("%.5f", val)})
}
